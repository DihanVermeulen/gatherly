...
type EventsAction =
  ...
  | { type: "SET_EVENTS"; payload: TEvent[] }
  | { type: "SET_LOADING"; payload: boolean }
  | { type: "SET_ERROR"; payload: string | null }
  | {
      type: "ADD_WISHLIST_ITEM";
      payload: { eventId: string; item: TWishlistItem };
    }
  | {
  ...
  | {
      type: "SET_WISHLISTS";
      payload: { eventId: string; items: TWishlistItem[] };
    };
...
export const EventsProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const handleWishlistAction = (action: EventsAction) => {
    queryClient.setQueryData<TEvent[]>(["events"], (old = []) => {
      switch (action.type) {
